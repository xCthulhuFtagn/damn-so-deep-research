...
from backend.core.llm import get_creative_llm

logger = logging.getLogger(__name__)

REPORTER_PROMPT = """You are the Reporter for a deep research system.

Your job is to create a COMPREHENSIVE and DETAILED research report based on the completed research.
...
YOUR TASK:
Create a well-structured, DETAILED Markdown report that:
1. Directly and thoroughly addresses the original query
2. Synthesizes ALL key findings from research steps with rich detail
3. Expands on important points with explanations, context, and implications
4. Clearly distinguishes between confirmed findings and partial/uncertain information
5. Explicitly states what could NOT be found (knowledge gaps)
6. Uses proper Markdown formatting (headers, lists, code blocks if relevant)
7. Cites sources where available
8. Provides actionable insights and conclusions

IMPORTANT: Write a COMPREHENSIVE report. Do not summarize too briefly.
...
- Provide context that helps understand the findings

REPORT STRUCTURE:
# Research Report: [Topic]

## Executive Summary
[Overview of key findings + notable gaps - 2-3 paragraphs]

## Detailed Findings
[Organized by theme, with thorough explanations and examples]
[Each section should be substantive - multiple paragraphs where appropriate]

## Analysis and Implications
[What do these findings mean? What are the implications?]

## Information Gaps
[What we tried but couldn't find - only if there are gaps]

## Key Insights and Recommendations
[Most important takeaways with actionable advice]
...
Be thorough but honest - partial information is better than pretending to have complete answers."""
...
*Generated by damn-so-deep-research*"""
        ...
        # Generate report via creative LLM (higher temperature, more tokens)
        llm = get_creative_llm(run_id=run_id)
        messages = [
            SystemMessage(
