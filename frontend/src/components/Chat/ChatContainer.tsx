    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-slate-200 bg-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold text-slate-900">Research Chat</h2>
          {isRunning && (
        ...
        <div className="flex items-center gap-2">
          {isRunning ? (
            <button
              onClick={onPause}
              className="flex items-center gap-2 px-4 py-2 text-sm bg-yellow-100 text-yellow-700 rounded-md hover:bg-yellow-200"
            >
        ...
        <div className="px-6 py-3 bg-primary-50 border-b border-primary-200">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {searchThemes.map((theme, i) => (
              <span
                key={i}
                className="px-2 py-1 bg-white rounded text-xs text-primary-700 border border-primary-200"
              >
                {theme}
              </span>
            ))}
