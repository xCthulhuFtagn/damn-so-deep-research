...
@function_tool
async def add_steps_to_plan(steps: List[str]) -> str:
    """
    ...
    count = 0
    max_num = await db_service.get_max_step_number(run_id)

    for desc in steps_list:
        if not desc: continue
        step_num = max_num + 1
        await db_service.add_plan_step(run_id, desc, step_num)
        max_num = step_num
        count += 1
...
@function_tool
async def get_current_plan_step() -> str:
    """
    ...
    step = await db_service.get_next_step(run_id)
    ...
    await db_service.set_active_task(run_id, int(step["step_number"]))
...
@function_tool
async def insert_corrective_steps(steps: List[str]) -> str:
    """
    ...
    try:
        await db_service.insert_plan_steps_atomic(run_id, steps_list, insert_after_step=active_task_num)
